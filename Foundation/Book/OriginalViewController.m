...
- (void)viewDidLoad {
    ...
    [self fetchData];
    self.navigationItem.title = @"原创";
}

//初始化代理
- (void)initDelegate {
    self.tableViewDelegate = [[OriginalDelegate alloc] init];
    self.tableViewDelegate.vc = self;
    //    先行强转
    OriginalDelegate *tableViewDelegate = (OriginalDelegate *)self.tableViewDelegate;
